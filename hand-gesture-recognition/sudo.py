# coding=utf-8
# Video sequence is just a collection of frames or collection of images that runs with respect to time. Make code
# stare at background without hand Bring hand in foreground with background Apply background-subtraction Thresholding
# is the assigment of pixel intensities to 0’s and 1’s based a particular threshold level so that our object of
# interest alone is captured from an image. Contour is the outline or boundary of an object located in an image.
...
def segmentation(image, threshold=25):
    ...
    diff = cv2.absdiff(background.astype("uint8"),
                       image)  # absolute difference between background and image(current frame)
    # print(diff)

    thresholded = cv2.threshold(diff, threshold, 255, cv2.THRESH_BINARY)[
        1]  # cv2.threshold() returns two o/p. First is retval and second is threshold image. Hence, we choose second val [1]
    # print(thresholded)

    (_, cnts, _) = cv2.findContours(thresholded.copy(), cv2.RETR_EXTERNAL,
                                    cv2.CHAIN_APPROX_SIMPLE)  # Finds contours in a binary image.
    # print(cnt)
...
def compute():
    ...
    while True:
        ...
        gray = cv2.GaussianBlur(gray, (7, 7),
                                0)  # (src ,kernel_size(height and width), sigmaX and sigmaY both set to 0)
        ...
        if num_frames < 30:
            ...
            if hand is not None:
                ...
                cv2.drawContours(clone, [segmented + (right, top)], -1, (
                    0, 0,
                    255))  # (destination_img, contours to draw, contourIdx(-1 denotes all contours are drawn), color)

                # count no of count_fingers
                fingers = count_fingers(thresholded, segmented)

                cv2.putText(clone, "Detected Value: " + str(fingers), (70, 45), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (0, 0, 255), 2)
